...
#include "Weapons/WeaponInstant.h"
...
const float AFusionHUD::MinHudScale = 0.5f;

AFusionHUD::AFusionHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	*/
	...
	OnPlayerTalkingStateChangedDelegate = FOnPlayerTalkingStateChangedDelegate::CreateUObject(this, &AFusionHUD::OnPlayerTalkingStateChanged);
	...
	HitNotifyIcon[EFusionHudPosition::Left] = UCanvas::MakeIcon(HitNotifyTexture, 158, 831, 585, 392);
	HitNotifyIcon[EFusionHudPosition::FrontLeft] = UCanvas::MakeIcon(HitNotifyTexture, 369, 434, 460, 378);
	HitNotifyIcon[EFusionHudPosition::Front] = UCanvas::MakeIcon(HitNotifyTexture, 848, 284, 361, 395);
	HitNotifyIcon[EFusionHudPosition::FrontRight] = UCanvas::MakeIcon(HitNotifyTexture, 1212, 397, 427, 394);
	HitNotifyIcon[EFusionHudPosition::Right] = UCanvas::MakeIcon(HitNotifyTexture, 1350, 844, 547, 321);
	HitNotifyIcon[EFusionHudPosition::BackRight] = UCanvas::MakeIcon(HitNotifyTexture, 1232, 1241, 458, 341);
	HitNotifyIcon[EFusionHudPosition::Back] = UCanvas::MakeIcon(HitNotifyTexture, 862, 1384, 353, 408);
	HitNotifyIcon[EFusionHudPosition::BackLeft] = UCanvas::MakeIcon(HitNotifyTexture, 454, 1251, 371, 410);
	...
	Crosshair[EFusionCrosshairDirection::Left] = UCanvas::MakeIcon(HUDMainTexture, 43, 402, 25, 9); // left
	Crosshair[EFusionCrosshairDirection::Right] = UCanvas::MakeIcon(HUDMainTexture, 88, 402, 25, 9); // right
	Crosshair[EFusionCrosshairDirection::Top] = UCanvas::MakeIcon(HUDMainTexture, 74, 371, 9, 25); // top
	Crosshair[EFusionCrosshairDirection::Bottom] = UCanvas::MakeIcon(HUDMainTexture, 74, 415, 9, 25); // bottom
	Crosshair[EFusionCrosshairDirection::Center] = UCanvas::MakeIcon(HUDMainTexture, 75, 403, 7, 7); // center

	Offsets[EFusionHudPosition::Left] = FVector2D(173, 0);
	Offsets[EFusionHudPosition::FrontLeft] = FVector2D(120, 125);
	Offsets[EFusionHudPosition::Front] = FVector2D(0, 173);
	Offsets[EFusionHudPosition::FrontRight] = FVector2D(-120, 125);
	Offsets[EFusionHudPosition::Right] = FVector2D(-173, 0);
	Offsets[EFusionHudPosition::BackRight] = FVector2D(-120, -125);
	Offsets[EFusionHudPosition::Back] = FVector2D(0, -173);
	Offsets[EFusionHudPosition::BackLeft] = FVector2D(120, -125);
...
void AFusionHUD::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	ConditionalCloseScoreboard(true);

	AFusionPlayerController* FusionPC = Cast<AFusionPlayerController>(PlayerOwner);
	if (FusionPC != NULL)
	{
		// Reset the ignore input flags, so we can control the camera during warmup
		FusionPC->SetCinematicMode(false, false, false, true, true);
	}
...
EFusionMatchState::Type AFusionHUD::GetMatchState() const
{
	return MatchState;
}

FString AFusionHUD::GetTimeString(float TimeSeconds)
{
...
void AFusionHUD::DrawWeaponHUD()
{
	AFusionCharacter* MyPawn = CastChecked<AFusionCharacter>(GetOwningPawn());
	AMasterWeapon* MyWeapon = MyPawn->GetCurrentWeapon();
	if (MyWeapon)
	{
		...
		//PRIMARY WEAPON
		{
			...
			const float AmmoPerIcon = MyWeapon->GetMaxAmmoPerClip() / MyWeapon->AmmoIconsCount;
		...
		//SECONDARY WEAPON
		AMasterWeapon* SecondaryWeapon = NULL;
		for (int32 i = 0; i < MyPawn->Inventory.Num(); i++)
		{
			if (MyPawn->Inventory[i] != MyWeapon)
			{
				SecondaryWeapon = MyPawn->Inventory[i];
				break;
			}
		}
		if (SecondaryWeapon)
		{
			...
			const float AmmoPerIcon = SecondaryWeapon->GetMaxAmmoPerClip() / SecondaryWeapon->AmmoIconsCount;
...
void AFusionHUD::DrawHealth()
{
	AFusionCharacter* MyPawn = Cast<AFusionCharacter>(GetOwningPawn());
	Canvas->SetDrawColor(FColor::White);
...
void AFusionHUD::DrawMatchTimerAndPosition()
{
	...
	if (MyGameState && MatchState == EFusionMatchState::Playing)
	{
	...
	if (MyGameState && MyGameState->RemainingTime > 0)
	{
		...
		Text = FString();
		AFusionPlayerController* MyPC = Cast<AFusionPlayerController>(PlayerOwner);
		if (MyPC && MyGameState && MatchState == EFusionMatchState::Playing)
		{
			AFusionPlayerState* MyPlayerState = Cast<AFusionPlayerState>(MyPC->PlayerState);
			if (MyPlayerState)
			{
...
void AFusionHUD::NotifyOutOfAmmo()
{
	NoAmmoNotifyTime = GetWorld()->GetTimeSeconds();
}

void AFusionHUD::NotifyEnemyHit()
{
...
void AFusionHUD::DrawDebugInfoString(const FString& Text, float PosX, float PosY, bool bAlignLeft, bool bAlignTop, const FColor& TextColor)
{
...
void AFusionHUD::DrawCrosshair()
{
	AFusionPlayerController* PCOwner = Cast<AFusionPlayerController>(PlayerOwner);
	if (PCOwner)
	{
		AFusionCharacter* Pawn = Cast<AFusionCharacter>(PCOwner->GetPawn());
		if (Pawn && Pawn->GetCurrentWeapon() && !Pawn->IsSprinting()
			&& (Pawn->IsZooming() || (!Pawn->IsZooming() && !Pawn->GetCurrentWeapon()->bHideCrosshairWhileNotAiming)))
		{
			const float SpreadMulti = 300;
			AWeaponInstant* InstantWeapon = Cast<AWeaponInstant>(Pawn->GetCurrentWeapon());
			AMasterWeapon* MyWeapon = Pawn->GetCurrentWeapon();
			const float CurrentTime = GetWorld()->GetTimeSeconds();
			...
			for (int32 i = 0; i< 5; i++)
			{
				if (MyWeapon && MyWeapon->UseCustomAimingCrosshair && Pawn->IsZooming())
				{
			...
			if (Pawn->IsZooming() && MyWeapon->UseLaserDot)
			{
				Canvas->SetDrawColor(255, 0, 0, 192);
				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Center],
					CenterX - (*CurrentCrosshair[EFusionCrosshairDirection::Center]).UL*ScaleUI / 2.0f,
					CenterY - (*CurrentCrosshair[EFusionCrosshairDirection::Center]).VL*ScaleUI / 2.0f, ScaleUI);
			}
			else
			{
				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Center],
				...
				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Left],
					CenterX - 1 - (*CurrentCrosshair[EFusionCrosshairDirection::Left]).UL * ScaleUI - CrossSpread * ScaleUI,
					CenterY - (*CurrentCrosshair[EFusionCrosshairDirection::Left]).VL*ScaleUI / 2.0f, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Right],
					CenterX + CrossSpread * ScaleUI,
					CenterY - (*CurrentCrosshair[EFusionCrosshairDirection::Right]).VL * ScaleUI / 2.0f, ScaleUI);

				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Top],
					CenterX - (*CurrentCrosshair[EFusionCrosshairDirection::Top]).UL * ScaleUI / 2.0f,
					CenterY - 1 - (*CurrentCrosshair[EFusionCrosshairDirection::Top]).VL * ScaleUI - CrossSpread * ScaleUI, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EFusionCrosshairDirection::Bottom],
					CenterX - (*CurrentCrosshair[EFusionCrosshairDirection::Bottom]).UL * ScaleUI / 2.0f,
...
void AFusionHUD::DrawDeathMessages()
{
	if (PlayerOwner == NULL)
	{
		return;
...
void AFusionHUD::ShowDeathMessage(class AFusionPlayerState* KillerPlayerState, class AFusionPlayerState* VictimPlayerState, const UDamageType* KillerDamageType)
{
	...
	{
		const AFusionGameMode_TeamDeathMatch* DefGame = GetWorld()->GetGameState()->GetDefaultGameMode<AFusionGameMode_TeamDeathMatch>();
		AFusionPlayerState* MyPlayerState = PlayerOwner ? Cast<AFusionPlayerState>(PlayerOwner->PlayerState) : NULL;
		...
		if (DefGame && KillerPlayerState && VictimPlayerState && MyPlayerState)
		{
			...
			NewMessage.DamageType = Cast<const UFusionBaseDamageType>(KillerDamageType);
...
void AFusionHUD::NotifyWeaponHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
	AFusionCharacter* MyPawn = (PlayerOwner) ? Cast<AFusionCharacter>(PlayerOwner->GetPawn()) : NULL;
	if (MyPawn)
	{
...
void AFusionHUD::DrawHitIndicator()
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
	if (CurrentTime - LastHitTime <= HitNotifyDisplayTime)
	{
...
void AFusionHUD::OnPlayerTalkingStateChanged(TSharedRef<const FUniqueNetId> TalkingPlayerId, bool bIsTalking)
{
...
void AFusionHUD::PostInitializeComponents()
{
...
void AFusionHUD::ConditionalCloseScoreboard(bool bFocus)
{
	if (bIsScoreBoardVisible)
	{
...
void AFusionHUD::ToggleScoreboard()
{
	ShowScoreboard(!bIsScoreBoardVisible);
}

bool AFusionHUD::ShowScoreboard(bool bEnable, bool bFocus)
{
	...
	{
		AFusionPlayerController* FusionPC = Cast<AFusionPlayerController>(PlayerOwner);
		if (FusionPC == NULL || FusionPC->IsGameMenuVisible())
		{
			return false;
		}
	}
...
void AFusionHUD::ToggleChat()
{
	AFusionPlayerController* FusionPC = Cast<AFusionPlayerController>(PlayerOwner);
	
	// If the game menu is visible dont show the chat menu
	if ((FusionPC == NULL) || FusionPC->IsGameMenuVisible() || GetMatchState() == EFusionMatchState::Warmup)
	{
		return;
	}
...
void AFusionHUD::SetChatVisibilty(const EVisibility RequiredVisibility)
{
...
void AFusionHUD::AddChatLine(const FText& ChatString, bool bWantFocus)
{
...
void AFusionHUD::MakeUV(FCanvasIcon& Icon, FVector2D& UV0, FVector2D& UV1, uint16 U, uint16 V, uint16 UL, uint16 VL)
{
	if (Icon.Texture)
	{
...
bool AFusionHUD::TryCreateChatWidget()
{
	...
	if (FusionPC == NULL)
	{
		UE_LOG(LogTemp, Warning, TEXT("Unable to create chat widget - Invalid player controller"));
	}
	/*
	else
	{
		...
		{
			bCreated = true;
			FLocalPlayerContext WorldContext(FusionPC);
			GEngine->GameViewport->AddViewportWidgetContent(
...
bool AFusionHUD::IsMatchOver() const
{
	return GetMatchState() == EFusionMatchState::Lost || GetMatchState() == EFusionMatchState::Won;
}

void AFusionHUD::AddMatchInfoString(const FCanvasTextItem InInfoItem)
{
	InfoItems.Add(InInfoItem);
}

float AFusionHUD::ShowInfoItems(float YOffset, float TextScale)
{
	float Y = YOffset;
...
float AFusionHUD::DrawRecentlyKilledPlayer()
{
