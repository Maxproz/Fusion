...
void AWeaponInstant::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
	...
	UFusionBaseDamageType* DmgType = Cast<UFusionBaseDamageType>(DamageType->GetDefaultObject());
...
void AWeaponInstant::ProcessInstantHitConfirmed(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir)
{
	// Handle damage
...
bool AWeaponInstant::ServerNotifyHit_Validate(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir)
{
	return true;
}


void AWeaponInstant::ServerNotifyHit_Implementation(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir)
{
...
bool AWeaponInstant::ServerNotifyMiss_Validate(FVector_NetQuantizeNormal ShootDir)
{
	return true;
}


void AWeaponInstant::ServerNotifyMiss_Implementation(FVector_NetQuantizeNormal ShootDir)
{
	const FVector Origin = GetMuzzleLocation();
...
void AWeaponInstant::SpawnTrailEffects(const FVector& EndPoint)
{
	...
	else
	{
		...
		AFusionCharacter* OwningPawn = GetPawnOwner();
...
void AWeaponInstant::OnRep_HitLocation()
{
...
void AWeaponInstant::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION(AWeaponInstant, HitImpactNotify, COND_SkipOwner);
}
