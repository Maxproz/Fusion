...
struct FDeathMessage
{
	...
	TWeakObjectPtr<class UFusionBaseDamageType> DamageType;
...
public:
	AFusionHUD(const FObjectInitializer& ObjectInitializer);
...
public:
	...
	*/
	...
	/** Sent from FusionWeapon, shows NO AMMO text. */
	...
	*/
	void SetMatchState(EFusionMatchState::Type NewState);

	/** Get state of current match. */
	EFusionMatchState::Type GetMatchState() const;
	...
	*/
	void ShowDeathMessage(class AFusionPlayerState* KillerPlayerState, class AFusionPlayerState* VictimPlayerState, const UDamageType* KillerDamageType);
