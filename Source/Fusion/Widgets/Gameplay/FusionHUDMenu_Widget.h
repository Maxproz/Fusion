...
public:
	...
	void UpdateArrows(TSharedPtr<FFusionMenuItemData> MenuItem);
	...
	TArray<FFusionMenuInfo> MenuHistory;
...
private:
	...
	FText GetOptionText(TSharedPtr<FFusionMenuItemData> MenuItem) const;
...
{
	FORCEINLINE void EnsureValid(TSharedPtr<FFusionMenuItemData>& MenuItem)
	{
		if (!MenuItem.IsValid())
		{
			MenuItem = FFusionMenuItemData::CreateRoot();
		}
	}

	//Helper functions for creating menu items
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuItem(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text));
	...
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuItem(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text, UserClass* inObj, typename FFusionMenuItemData::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text));
	...
	template< class UserClass >
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuItemSP(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text, UserClass* inObj, typename FFusionMenuItemData::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text));
	...
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuOption(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text, const TArray<FText>& OptionsList)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text, OptionsList));
		MenuItem->SubMenu.Add(Item);
	...
	template< class UserClass >
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuOption(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FFusionMenuItemData::FOnOptionChanged::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text, OptionsList));
	...
	template< class UserClass >
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddMenuOptionSP(TSharedPtr<FFusionMenuItemData>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FFusionMenuItemData::FOnOptionChanged::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFusionMenuItemData> Item = MakeShareable(new FFusionMenuItemData(Text, OptionsList));
	...
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddExistingMenuItem(TSharedPtr<FFusionMenuItemData>& MenuItem, TSharedRef<FFusionMenuItemData> SubMenuItem)
	{
		EnsureValid(MenuItem);
	...
	FORCEINLINE TSharedRef<FFusionMenuItemData> AddCustomMenuItem(TSharedPtr<FFusionMenuItemData>& MenuItem, TSharedPtr<SWidget> CustomWidget)
	{
		EnsureValid(MenuItem);
		MenuItem->SubMenu.Add(MakeShareable(new FFusionMenuItemData(CustomWidget)));
		return MenuItem->SubMenu.Last().ToSharedRef();
	}

	FORCEINLINE void ClearSubMenu(TSharedPtr<FFusionMenuItemData>& MenuItem)
	{
		EnsureValid(MenuItem);
	...
	template< class UserClass >
	FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, typename FFusionMenuItemData::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
