...
DECLARE_DELEGATE_RetVal_OneParam(int32, FOnGetPlayerStateAttribute, AFusionPlayerState*);
...
//class declare
class SScoreboard_Widget : public SBorder
{

	SLATE_BEGIN_ARGS(SScoreboard_Widget)
	{}

	SLATE_ARGUMENT(TWeakObjectPtr<APlayerController>, PCOwner)

	SLATE_ATTRIBUTE(EFusionMatchState::Type, MatchState)
...
protected:
	...
	FSlateStyleSet Style = ("FusionStyle");
	...
	AFusionPlayerState* GetSortedPlayerState(const FTeamPlayer& TeamPlayer) const;
	...
	int32 GetAttributeValue_Kills(class AFusionPlayerState* PlayerState) const;

	/** get attribute value for deaths */
	int32 GetAttributeValue_Deaths(class AFusionPlayerState* PlayerState) const;

	/** get attribute value for score */
	int32 GetAttributeValue_Score(class AFusionPlayerState* PlayerState) const;
	...
	EFusionMatchState::Type MatchState;

	/** pointer to our parent HUD */
	TWeakObjectPtr<class APlayerController> PCOwner;

	/** style for the scoreboard */
	const struct FFusionScoreboardStyle *ScoreboardStyle;
};
