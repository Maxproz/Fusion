...
#include "FusionPlayerCameraManager.h"


AFusionPlayerCameraManager::AFusionPlayerCameraManager(const class FObjectInitializer& ObjectInitializer)
...
void AFusionPlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();
...
void AFusionPlayerCameraManager::UpdateCamera(float DeltaTime)
{
	AFusionCharacter* MyPawn = PCOwner ? Cast<AFusionCharacter>(PCOwner->GetPawn()) : nullptr;
	if (MyPawn)
	{
		// TODO: Replace this with Weapon Zooming code
	...
	if (MyPawn)
	{
		...
		FVector CurrentCameraOffset = MyPawn->GetFirstPersonCameraComponent()->GetRelativeTransform().GetLocation();
		FVector NewCameraOffset = FVector(CurrentCameraOffset.X, CurrentCameraOffset.Y, DefaultCameraOffsetZ + CurrentCrouchOffset);
		MyPawn->GetFirstPersonCameraComponent()->SetRelativeLocation(NewCameraOffset);
	}
