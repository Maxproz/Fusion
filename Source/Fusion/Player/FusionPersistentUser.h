...
#include "FusionPersistentUser.generated.h"

/**
 * 
 */
UCLASS()
class FUSION_API UFusionPersistentUser : public USaveGame
{
	GENERATED_BODY()
	
public:

	UFusionPersistentUser(const FObjectInitializer& ObjectInitializer);

	/** Loads user persistence data if it exists, creates an empty record otherwise. */
	static UFusionPersistentUser* LoadPersistentUser(FString SlotName, const int32 UserIndex);
